...
- [**#11** 인천항 컨테이너 터미널별 처리량: 합계 한 줄은 다섯 곳을 대표하지 않는다 (2025-10~2026-07)](reports/report_11_터미널별_처리량.md)
  축을 항 전체에서 **터미널 층**으로 내렸다. 기준 4개 중 T1·T2 PASS, T3·T4 FAIL. 공표 합계의 전년대비 부호가 터미널 다섯 전부와 같은 달은 10개월 중 1개월. **창이 10개월인 것은 원천의 한계가 아니라 우리 수집기가 목록 첫 쪽만 읽어서였다** — 그 사실과 **고친다는 것**을 편 안에 적었다.
- [**#12** 인천항 컨테이너 터미널별 처리량, 220개월 중 39개월: 기준 셋을 다 못 넘었다 (2023-03~2026-07)](reports/report_12_터미널별_장기창.md)
  **220은 게시판에 글이 있는 달의 수다**(구간 폭 239개월 중 19개월은 글이 없다). #11 이 예고한 대로 **수집기를 고쳐 같은 원천을 39개월로 다시 쟀다.** 창 41개월 / 관측 39 / 판정 불가 두 달. 기준 셋(S1 계절성·S2 쌍 동행·S3 몫의 단조성) **전부 FAIL** — 그리고 **셋 다 선커밋이 미리 적어 둔 귀무기대와 가까운 값**이었다. **기준마다 사전 난이도를 기준과 같이 커밋한 첫 편이다.**
...
[**인천항 컨테이너 물동량**](reports/물동량.md)은 번호가 붙는 판정형 편이 아니라, 공식 1차 데이터를 그대로 정리해 두는 정리형 스냅샷이다. 헤드라인·최근 12개월 추이·터미널별·공컨테이너 수출입을 담고, 주 1회 클라우드 루틴이 새 월 공표가 나오면 그 값으로, 안 나온 주에는 다른 단면으로 고쳐 쓴다. 절차 = `.claude/skills/주간물동량/SKILL.md`, 판정 기준 = [`docs/측심_주간루틴_판정기준_선커밋_20260924.md`](docs/측심_주간루틴_판정기준_선커밋_20260924.md).
...
10. **#11 층을 내린다 · 우리 쪽 한계 공개** 아홉 편이 본 「항 전체 한 줄」을 **터미널 다섯으로 갈라** 그 한 줄의 대표성을 쟀다(부호 동행 10개월 중 1개월 · T3 FAIL). 그리고 **이 편의 창이 원천이 아니라 우리 수집기 때문에 10개월이었다는 것**을 편 안에서 밝히고 고치기로 했다.
11. **#12 예고를 이행한다 · 기준의 난이도를 기준과 같이 커밋** 수집기를 고쳐 **같은 원천을 39개월로 다시 쟀다.** 새 기준 셋을 세우고 **셋 다 FAIL** 로 냈으며, **각 기준이 얼마나 어려운지를 판정 전에 같이 커밋했다** — 그래서 「FAIL 을 연출했다」가 성립하지 않는다(S2 는 사전 통과 가능성이 절반인 기준이었다).
...
│   ├── report_11_터미널별_처리량.md
│   └── report_12_터미널별_장기창.md

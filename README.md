# 인천항 공컨테이너 물동량 분석 — 2025년 월별 동향 (보고서 #01)
...
전월 대비 증감률은 전체 공컨 물동량 기준이며, `container_2025.csv`에서 실제 계산한 값이다.
...
이번 분석은 **공(빈)컨테이너만** 다뤘다는 명확한 한계가 있다. 공컨 물동량은 전체 컨테이너 흐름의 한 단면일 뿐이어서, 위 3번 해석의 핵심인 '수출입 불균형'을 직접 확인하려면 화물이 실린 적(積)컨테이너를 포함한 전체 물동량과 함께 봐야 한다. 다음 **보고서 #02**에서는 인천항 전체 컨테이너 물동량 대비 공컨 비율을 산출해, 재배치 부담의 규모와 방향을 정량적으로 검토할 예정이다.
...
검증을 거쳐 조건에 맞는 공컨테이너 통계 API를 선정했다. 원본 응답은 한 달에 반입·반출 구분과 터미널 구분이 나뉘어 **64행**이었고, 월(mm) 기준으로 합산해 **12행**으로 집계했다.
...
1. **인증키 발급** — 공공데이터포털(data.go.kr)에서 인천항만공사 공컨테이너 화물 통계정보 API 활용 신청 후 인증키를 받는다.
2. **인증키 설정** — `config_example.py`를 복사해 파일명을 `config.py`로 바꾸고, 발급받은 인증키를 넣는다.
   ```
   # config_example.py 를 config.py 로 복사한 뒤, 아래 값을 본인 키로 교체
   SERVICE_KEY = "여기에_본인의_공공데이터포털_인증키를_넣으세요"
   ```
   > `config.py`는 `.gitignore`로 제외되어 저장소에 올라가지 않는다 — 인증키 노출 방지를 위한 구조다.
3. **라이브러리 설치**
   ```

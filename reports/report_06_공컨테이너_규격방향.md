# #06 인천항 공컨테이너 규격×방향 비대칭 — 수입 공컨은 40ft로 쏠린다 (2022–2025)
...
- **작성일**: 2026-07-22
- **분석 대상**: 2022~2025년 4개년(48개월) 인천항 공(빈)컨테이너의 규격(10/20/40ft·기타)×방향(수입/수출) 구성, 박스 수 기준, 모집단 ocCt=1(수출입항)
- **한 줄 결론**: 빈 컨테이너의 규격 구성은 수입과 수출의 방향에 따라 다른 모습을 보인다. 2025년 수입 기준으로 40ft의 비중은 91.1%이고 수출 기준으로는 52.4%로 수입 기준이 38.7%p 높다(박스 수 기준). 이러한 결과는 48개월 중 42개월 동안 성립된 것이다.

---

## 1. 핵심 요약

- 빈 컨테이너의 규격 구성은 수입과 수출의 방향에 따라 다른 모습을 보인다.
- 2025년 수입 기준으로 40ft의 비중은 91.1%이고 수출 기준으로는 52.4%로 수입 기준이 38.7%p 높다(박스 수 기준). 이러한 결과는 48개월 중 42개월 동안 성립된 것이다.
...
성립되지 않은 6개월은 모두 2022년으로 2, 3, 5, 7, 8, 9월이다. 2022년 10월부터 2025년 12월까지 39개월 연속으로 성립되었다. 각 연도마다 격차는 +7.5, +33.0, +37.8, +38.7%p로 증가하는 모습이 보였다.

### 3-2. 설명 가설

수입 공컨과 특정 규격 수요가 연결될 가능성이 있다는 가설을 세울 수 있다. 선사·화주의 방향별 규격 운용 관행의 가능성도 존재한다. 그러나 본 데이터로는 검증할 수 없다.

### 3-3. 이 데이터로 판별할 수 없는 것

위 데이터로 알 수 없는 것으로는 기타 규격(_99)의 정의 미확인, 선사·부두 단위 분해 불가, 쏠림의 원인 규명 불가가 있다.

## 4. 방법론

- **데이터·모집단**: 공컨테이너 화물 통계 API 원시 CSV 4개년(`analysis/container_2022~2025_direction.csv`). 모집단은 수출입항(ocCt=1), 외국적+한국적 합산. 단일 소스로 완결되며 이질 소스 결합이 없다.
...
- **규격식 검산**: `0.5×_10 + 1×_20 + 2×_40 + 2.25×_99 = TEU`가 수출입항 전 행(4개년 192행 × 외국적·한국적 = 384건)에서 최대편차 0으로 성립함을 확인했다.
...
- **후속 보고서**: 이 보고서가 관측한 격차의 부호가 2026년 상반기에도 성립하는지를 [보고서 #07](report_07_공컨테이너_표본외검증.md)에서 표본외 검증했다. 판정 기준을 데이터 수집 전에 커밋했고, 부호 연속 성립 기록은 45개월로 연장됐다.
...
- 2026-08-25: `ocCt=1` 표기를 공식 라벨 '수출입항'으로 통일(표 1 제목·방법론 2곳). 근거 = `docs/GInOut_코드규명.md` §3 [확정]. 수치 변경 없음.

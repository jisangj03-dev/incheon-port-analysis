...
>
> **제목이 말하는 「220개월 중 39개월」의 범위는 하나다 — 우리가 읽은 달의 수다.** 그리고 **220은 구간 폭이 아니라 게시판에 글이 있는 달의 수**다.<sup>3</sup> 구간은 2006-09~2026-07, **폭은 239개월**이고 그중 **19개월은 게시판에 글이 없다**(239 − 19 = 220). 글이 있는 220개월 가운데 **2023-02 이전은 첨부가 `.hwp`·`.xls` 라 우리 파서가 못 연다** — 「원문에 없다」가 아니다. **읽는 일은 이 편에 넣지 않았다**(§4.2). 그리고 **#11 의 기준(T1~T4)을 이 창에 옮겨 쓰지 않았다** — 새 기준 셋을 새로 커밋했다.
...
**고친 뒤 원천을 다시 쟀다.** 목록은 **23쪽**이고 가장 이른 글은 2006년 9월분, 가장 늦은 글이 2026년 7월분이다 —
**구간 폭 239개월 · 글이 있는 달 220개월 · 글이 없는 달 19개월**이다(재측정 2026-09-13).
...
| └ 창 안에서 못 읽은 달 | 두 달 (2023-12·2024-01) — 첨부가 `.hwp` | — |

**「220개월」과 「239개월」은 다른 수다.** 220은 **글이 있는 달**, 239는 **두 끝 사이의 폭**이다.
**제목의 분모는 220**이다 — 없는 글을 못 읽은 것으로 세지 않는다.
**「글이 없다」와 「우리가 못 읽는다」도 다르다** — 앞은 원문이 없고, 뒤는 원문이 있는데 우리 파서가 못 연다.
...
- **못 읽은 구간에 무엇이 있는지 모른다.** **글이 있는 220개월 중 181개월**을 우리 파서가 못 연다
  (구간 폭 239개월 중 19개월은 글 자체가 없고, 그쪽은 이 수에 안 들어간다).
...
- **글이 있는 220개월 중 39개월만 읽었다(17.7%).** 2023-02 이전은 첨부가 `.hwp`·`.xls` 다.
  **구간 폭 239개월 중 19개월은 게시판에 글이 아예 없다** — 그 19개월은 분모에 안 넣었다.
...
| 창의 출처 — 게시판 깊이·폭·빠진 달 (값 비노출) | `analysis/probe/probe_15_board_depth.py` · 전 쪽은 `--full` |
...
3. **정정 [2026-09-13].** 이 리드는 발행 당시 **「게시판에는 220개월이 다 있고」**로 나갔다. **거짓이다** — 2006-09~2026-07 의 **폭은 239개월**이고 그중 **19개월은 게시판에 글이 없다**(`2007-01 · 2012-02 · 2014-08~10 · 2015-03 · 2016-11~12 · 2017-12 · 2018-01 · 2018-08~2019-03 · 2022-09`). **220은 글이 있는 달의 수**이고 제목의 분모도 그것이다. **두 끝을 뺀 독자가 239 를 얻고 220 과 못 맞추던 자리**이며, 그 셋(폭·글·빠진 달)을 이제 §2.1 표가 같이 든다. 재측정·재현 = `python analysis/probe/probe_15_board_depth.py --full`(`span_gaps()` 가 셋을 같이 낸다). **판정값에는 영향이 없다** — 창은 2023-03~2026-07 그대로이고 19개월은 그 창 밖이다.

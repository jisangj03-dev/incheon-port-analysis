...
⑤ 이로써 #01(관찰) → #02(검증) → #03(규명) → #04(지속성)의 연구 체인이 닫힌다. 다음 질문은 "빈 컨테이너가 수출로 쏠리는 만큼, 화물이 든 적컨은 반대로 수입에 쏠리는가"이며, [보고서 #05](report_05_컨테이너_수지.md)가 이를 확인했다. (당초 이월했던 "어느 부두가 지는가"는 공컨 × 부두 교차표가 어느 공개 데이터원에도 없어 본 시리즈에서 다루지 못한다 — [docs/05_주제검증.md](../docs/05_주제검증.md) §0 참조.)

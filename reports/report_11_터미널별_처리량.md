...
| 기준 | 판정 대상 | 통과 조건 |
|---|---|---|
| T1 몫 순위 지속 | 10개월 각각 | 신항 > 남항 > 국제여객부두 |
| T2 신항 몫 하한 | 10개월 각각 | 신항 몫 ≥ 50.0% |
| T3 증감 동행 | 10개월 | 합계와 터미널 다섯의 전년대비 부호가 모두 같은 달 ≥ 6개월 |
| T4 안벽 생산성 순위 지속 | 10개월 각각 | 대응 터미널의 1m당 처리량 순위가 내내 같다 |
...
| 기준 | 결과 | 판정 |
|---|---|---|
| T1 몫 순위 지속 | 10/10 성립 | **PASS** |
| T2 신항 몫 ≥ 50.0% | 10/10 성립 (63.4~68.8%) | **PASS** |
| T3 증감 동행 | 1개월 (기준 6개월 이상) | **FAIL** |
| T4 안벽 생산성 순위 지속 | 순위 2종 (9개월 + 1개월) | **FAIL** |
...
E1CT는 10개월 전부 양(+)으로, SNCT는 9개월이 음(−)으로 산출됐다. 그 사이에서 합계 부호는 6번 바뀐다.
...
*부두를 안벽 길이(m)에 비례한 막대로 그리고, 채움 농도에 월평균 1m당 천TEU를 얹었다. **배치는 모식이다** — 부두의 평면 좌표는 공표되지 않는다. 길이만 비례하고 위치는 아니다.*
...
**이 값은 효율이 아니다.**<sup>1</sup> 선석 수·장비·선형·운영 시간이 전부 빠져 있다.
...
**두 표의 터미널 이름 집합이 다른 이유는 원문 안에 있다.**<sup>2</sup> 부두현황의 `주요취급화물` 열이
...
- **T2가 약한 기준이었다.** 위 3-2 그대로다.

### 4.2 후속

- **관측 최소 63.4%를 근거로 한 좁은 기준.** 이 편이 그 근거값을 만들었다.
- **터미널 축의 창 넓히기.** 같은 소스의 더 이른 달이 공표돼 있는지 확인이 필요하다.
...
1. 안벽 1m당 처리량은 **월평균 천TEU를 안벽 길이 합으로 나눈 값**이다. 선석 수·하역 장비·기항 선형·운영 시간이 분모에 들어 있지 않으므로 효율·경쟁력·우위의 근거가 아니다. E1CT는 안벽이 259 m로 가장 짧고, 짧은 분모는 같은 물동량에서 큰 값을 낸다.
2. 이 편의 선커밋 문서는 두 표의 이름 집합 차이를 「이미 본 어긋남」으로 적었으나, 판정 과정에서 그것이 어긋남이 아니라는 것이 원문 안에서 드러났다. 정정 경위는 `docs/11_판정결과.md` §2에 있다.
3. 2026년은 잠정치, 2025년 10~12월은 확정치다. 두 값의 성격이 다르다.

...
| -----: | ------------: | ----------------: | ----------------: | ----------: |
|   2022 |       851,775 |   782,668 (91.9%) |            69,107 | 8.1% (수출입항의 2.17%) |
|   2023 |       773,831 |   711,761 (92.0%) |            62,070 | 8.0% (수출입항의 1.79%) |
|   2024 |       832,886 |   797,954 (95.8%) |            34,932 | 4.2% (수출입항의 0.98%) |
...
3. **초과분 분해의 잔차**: 「전체 순수입」(2022년 69,107 TEU, 수출입항의 2.17% 등)에는 장치장 재고 변동·육상 반출입·소스 간 집계 기준차 등 여러 가설이 가능하나 본 데이터로는 판별 불가하다. 인천항의 컨테이너는 육상·타 항만으로도 이동하므로 단일 항만의 수지가 닫혀야 할 물리적 이유는 없다.
...
- **후속 보고서**: 공컨테이너의 규격(사이즈)×방향 비대칭은 [보고서 #06](report_06_공컨테이너_규격방향.md)에서 확인했다. 부두별 구성(신항/남항/국제여객부두)은 공컨 × 부두 교차표가 공개 데이터에 없어 유보 상태다([docs/05_주제검증.md](../docs/05_주제검증.md) §0).
...
- 2026-08-25: `ocCt=1` 표기를 공식 라벨 '수출입항'으로 통일(표 3의 3곳·한계 1곳). 근거 = `docs/GInOut_코드규명.md` §3 [확정]. 수치 변경 없음.

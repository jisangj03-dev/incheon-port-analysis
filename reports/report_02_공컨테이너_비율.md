...
- **후속 과제**: 적컨(= 전체 − 공컨)의 수출/수입 방향 분해 → 이 질문은 [보고서 #03](report_03_공컨테이너_수출입방향.md)이 공컨 방향 분해로 규명했다(적컨 직접 분해는 공개 데이터에 방향 축이 없어 불가 판정). 2024년 등 타 연도와의 비교로 28.8%가 추세적으로 오르는지/내리는지 확인.

...
지침 §1.4 1층은 편마다 **「결론 자리 수치 1건 이상 원본 대조」**를 요구하고, 그 대조가
남는 자리가 `docs/검수기록.md` 다(`review_pick.py --log`). 기록 없이 나가면 §2.2 표기 블록이
**일어나지 않은 검수를 증언한다** — 표기 의무가 막으려던 실패 그 자체다.

**[지침 v7.0] 검수 자체는 더 이상 운영자 전용이 아니다.** 대조는 **하는 쪽이 한다**(§2.3).
이 검사가 보는 것은 「누가 했나」가 아니라 **「기록이 있나」**라서 그 개정에 안 흔들린다.
...
**기본이 경고인 이유** [v7.0 재작성]. 종전 이유는 「막는 대상이 운영자의 손이라서」였는데
**push 는 이제 세션이 친다** — 그 이유는 사라졌다. 대신 난간 전체에 한 기준을 쓴다:
**push 가 되돌릴 수 없게 만드는 것만 막는다.** 검수 기록 누락은 push 뒤에도 고쳐진다
(`review_pick.py --log` 를 돌리고 커밋하면 된다). 그래서 경고다. 차단은 `--strict`.
**이 기준으로 차단인 것은 난간 ⑦(공개 유출) 하나뿐이다** — 유출은 이력에 남아 안 고쳐진다.
...
1. **수정된(M) 보고서는 안 본다.** 오탈자 정정까지 재검수를 요구하면 §3-11(형식 변경은 정정
   사유가 아니다)과 어긋나고 오탐이 난다. **그래서 발행본 개정에 대한 재검수는 판단이다.**

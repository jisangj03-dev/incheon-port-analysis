...
- **부두군도 마찬가지다.** 신항과 남항은 **안벽 길이·수심·접안능력이 다르다** —
  얼마나 다른지는 [부두 구조]({{{{ '/berths/' | relative_url }}}})가 공표 자료로 든다.
...
{body}
  ...
  <p class="tnote">
    ...
    (TEU 기준은 순환 논증이라 쓰지 않는다).
  </p>
  <div class="callout warn">
    <span class="k">[2026-08-30] 이 표는 4개년이고, 소스는 더 길다</span>
    <p>같은 소스에 <b>2005년치까지</b> 있다. 우리는 오래 <b>2022년부터라고 알고</b> 있었는데,
    그 믿음의 근거가 된 조회가 <b>2022년 위쪽만</b> 봤기 때문이었다.</p>
    <p><b>그래서 이 표의 {first}년을 「유난히 높은 해」로 읽으면 안 된다.</b>
    뒤로 늘려 판정해 보니 판정 가능 구간에서 <b>47.20배</b>·<b>54.66배</b>·<b>44.57배</b>인
    해가 있었다. <b>왜 그런지는 이 데이터로 판별할 수 없고 그래서 쓰지 않는다.</b></p>
    <p>판정 기준과 결과 — 그중 둘은 <b>FAIL</b>이다 — 는 <b>#09</b>가 든다.</p>
  </div>
...
def selftest() -> int:
    ...
    csv_names = {r["터미널"] for m in load()[1].values() for r in m.values()}
    chk("데이터의 이름이 공표 코드·집계명뿐이다 (허용 목록 · §4.1-1)",
        csv_names <= set(TERMS) | {TOTAL, OTHER, "신항", "남항"}, True)

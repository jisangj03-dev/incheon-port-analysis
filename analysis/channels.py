...
CHANNELS = {
    "linkedin": {
        ...
        "자기것허용": True,
        "링크카드": "URL 하나에 카드가 붙는다. **어느 것인지는 [미확인]** — 순서를 바꾸면 바뀐다",
        "사람이할일": "로그인 · 게시 버튼 · 카드 미리보기 확인",
        "메모": [
            "**앞 140자가 전부다.** 그 뒤는 「…더 보기」를 눌러야 보인다 — 결론을 앞에 둔다.",
            "URL 이 여럿이면 그중 **하나**에만 카드가 붙는다. **어느 것인지는 [미확인]이다.**",
            "  *실측 둘이 갈린다(2026-09-10). li-02 작성창: [측심, #09] 순서에서 카드가 **#09**(마지막),"
            "  순서를 뒤집기 전 [#09, 측심] 에서는 **측심**(마지막) — 마지막으로 보였다."
            "  그런데 게시된 li-01 은 [측심, 인천Pages] 인데 카드가 **측심**(첫째)이다."
            "  **어느 쪽 규칙으로도 둘을 설명 못 한다 — 그래서 단정하지 않는다.**"
            "  확실한 것 하나: **순서를 바꾸면 카드가 바뀐다.**)*",
            "**그러므로 붙여넣은 뒤 미리보기를 눈으로 보는 것이 유일한 확인이다**(사고 73).",
...
def check_format(channel, body, title=None):
    ...
    if c.get("링크카드") and len(us) > 1 and channel.lower() == "linkedin":
        # **어느 URL 에 카드가 붙는지는 [미확인]이다**(위 메모의 실측 둘이 갈린다).
        # 확실한 것은 「순서를 바꾸면 바뀐다」뿐이라 **보라고만** 한다.
        out.append((INFO, "URL %d개 — 카드는 **그중 하나**에만 붙는다. 어느 것인지는 [미확인]이니 "
                          "미리보기를 눈으로 본다" % len(us)))

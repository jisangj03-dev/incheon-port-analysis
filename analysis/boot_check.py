...
STATE = (
    ...
    ("check_refs.py", ()),
    # **[2026-09-10] 번호가 아니라 「주장」이 낡은 자리.** 사고 108 의 처분을 규율로 뒀는데
    # §0-3 이 그것을 금지한다 — 사람이 나르는 규칙은 유실된다(사고 22). 코드로 내렸다.
    ("check_claims.py", ()),
    # **[2026-09-10] 무엇이 세션에게 지시가 되고 있는가.** G5 가 금지 축에서 가시성 축으로
    # 바뀌면서(선커밋 `ea7b33e9`) 이 장치가 그 개정의 전제가 됐다 — 「보이니까 괜찮다」는
    # **보는 장치가 있을 때만** 참이다. 기준선과 어긋나면 종료 1 이라 개시 검사가 멈춘다.
    ("check_session_surface.py", ()),
    # **[2026-09-10] 내보내는 것에 표식이 묻어 나가는가.** 운영자 판단으로 상시 자리가 됐다 —
    # 채널 문안·발행본·사이트 지면·측심 카피, 예외 없음. **표식은 기계용 흔적이고
    # 「AI 를 썼다」는 사람용 고지라 둘은 별개다**(§2.2 는 그대로 둔다).
    ("check_marks.py", ()),
    # **[2026-09-12] `check_generated.py` 를 뺐다 — 파일째 없앴다.** 등록된 생성기
    # 다섯이 **전부** 한 번도 push 된 적 없는 허브로 나가고 있었다(사고 115).
    # **살아 있는 생성물(도판·차트 PNG)에는 신선도 검사가 없다** — 없던 것이고 지금도 없다.
    ("check_facts.py", ()),
    ("check_a11y.py", ()),
    ("check_counts.py", ()),
    ...
    ("check_private.py", ()),
    ("lint_publish.py", ()),
    # **[2026-09-12] 허브 지면 모드를 뺐다.** 사고 69 가 만든 지면 모드는 옳았는데
    # **그 지면이 죽었다**(사고 115). 살아 있는 지면은 측심 하나이고 그쪽은
    # `check_claims.py` 와 `check_site_facts.py` 가 본다. **대상이 죽으면 검사도 죽는다.**
    ...
    # **[2026-09-09] 채널 대기열.** 게시는 세션 하나로 안 끝난다 — 문안을 쓰는 세션과
    # 누르는 순간이 다른 시각에 있고 그 사이에 세션이 끊긴다(사고 96). 무엇이 대기 중인지
    # **개시할 때 보이게** 둔다. 보고일 뿐이라 종료코드 0 이고 마지막 줄이 합계를 든다.
    ("channel_queue.py", ()),
)

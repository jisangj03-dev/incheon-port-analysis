...
이 스크립트가 하는 일: 근거 구간 48개월의 월별 배율·수출비중 하한을 원시 CSV 에서 다시 계산한다.
닿지 않는 곳: 근거 구간 값이 맞는지는 #04 발행분과의 대조로만 본다. 판정 구간은 안 본다.

**정지선을 집행하지 않는다.** 그래서 `정지선-집행:` 표지를 안 단다 —
달면 STATUS 정지선 대조표에 가드로 올라가고, **그 표에 비-가드가 끼면 표가 묽어진다.**

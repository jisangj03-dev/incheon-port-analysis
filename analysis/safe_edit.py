...
**[지침 v7.0] `정지선-집행/명제/한계` 3줄을 뗐다.** 종전 §4 는 「치환은 이 도구로만」이라는
**도구 지정**이었고, 그건 정지선이 아니라 판단이다 — 잘못된 치환은 되돌릴 수 있다.
v7.0 §4-다가 남긴 것은 도구가 아니라 **성질**이다: 치환은 **실패가 드러나는 경로로만** 한다.
이 도구는 그 경로를 제공하지만 **유일한 경로가 아니다.** 그래서 정지선표에서 내려왔다.

**그 성질이 왜 필요한가는 안 바뀌었다** — 일치 개수를 안 세는 치환은

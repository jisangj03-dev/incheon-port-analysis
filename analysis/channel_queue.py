...
---------
  ...
  게시됨    나갔다. `url` 이 있다 — **다만 게시 지면이 없는 채널(1:1 서신)은 주소가 없다**
  보류     안 올린다. `note` 에 이유
...
def setrow(rid, state=None, url=None, note=None, verify=None):
    ...
    if state:
        if state not in STATES:
            ...
            return 2
        ch = CH.CHANNELS.get(r.get("채널"), {})
        # **주소를 요구하는 이유는 「확인은 주소를 여는 것」이기 때문이다**(이 파일 머리말).
        # 그런데 **게시 지면이 없는 채널**이 있다 — 1:1 서신이 그렇다. 거기서 주소를 강요하면
        # 가짜 주소가 들어오고, 그 순간 이 규칙이 지키려던 것이 없어진다.
        # **채널이 지면을 갖는지는 채널 규칙이 든다**(`channels.py` 의 `게시지면`).
        if state == "게시됨" and ch.get("게시지면", True) and not (url or r.get("주소")):
            print("**「게시됨」에는 주소가 있어야 한다** — `--url` 을 준다.")
            return 2
        if state == "게시됨" and not ch.get("게시지면", True):
            print("  (기록) %s 는 **게시 지면이 없는 채널**이다 — 열어 볼 주소가 없다."
                  % r.get("채널"))
            print("  **그래서 「게시됨」을 밖에서 확인할 길이 없다.** 무엇을 보냈는지는")
            print("  메모의 정본 해시가 들고, 확인은 회신으로 온다.")
...
def selftest():
    ...
    try:
        with tempfile.TemporaryDirectory() as d:
            ...
            print("── 인수시험: 게시 지면이 없는 채널 (1:1 서신) ──")
            src3 = [{"id": "m-01", "채널": "email", "무엇": "기고 문의", "상태": "준비",
                     "검증": "WARN(무망)", "문안": "mail-01.md", "주소": "", "갱신": "", "메모": ""},
                    {"id": "li-09", "채널": "linkedin", "무엇": "x", "상태": "준비",
                     "검증": "", "문안": "", "주소": "", "갱신": "", "메모": ""}]
            write(src3, head, tail)
            chk("지면이 없는 채널은 주소 없이 게시됨이 된다",
                setrow("m-01", state="게시됨"), 0)
            chk("지면이 있는 채널은 여전히 주소를 요구한다",
                setrow("li-09", state="게시됨"), 2)
            back, _, _ = read()
            chk("메일은 주소가 빈 채로 게시됨",
                [(x["상태"], x["주소"]) for x in back if x["id"] == "m-01"], [("게시됨", "")])
            chk("링크드인은 안 바뀌었다",
                [x["상태"] for x in back if x["id"] == "li-09"], ["준비"])

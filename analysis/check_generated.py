...
닿지 않는 곳
------------
1. ~~**허브 저장소에서는 안 돈다.**~~ [2026-09-03 닫힘] `git_pre_push.sh` 의 `find_guard()` 가
   형제 폴더에서 이 파일을 찾으므로 허브·측심을 push 할 때도 돈다. 경로는 `__file__` 로
   잡아 어디서 불러도 같은 것을 본다. (그 전엔 도구를 만들고 이웃에 안 번진 상태였다 — 사고 87.)
